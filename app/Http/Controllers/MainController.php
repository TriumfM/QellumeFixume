...
use DB;
use App\Events\getTries;
use App\Events\getRooms;
use App\Events\getPlayers;
use App\Events\getOrderInRoom;
...
class MainController extends Controller
{
    public function createRoom(Request $request){
        ...
        broadcast(new getRooms())->toOthers();
    ...
    public function enterRoom($id){
        ...
        $room = Room::where('id',$id)->with('games')->first();
        broadcast(new getPlayers($room->id))->toOthers();
    ...
    public function try(Request $request){
        ...
        $length =(int) $request->lengthNumber;
        $this->validate($request,[
            'number' => ['required','size:'.$length],
        ]);
    	...
    	$try->save();
        // broadcast(new getOrderInRoom($Ruaj))->toOthers();
        broadcast(new getTries($room->id))->toOthers();
    	return  array('room' => $room ,'fitore'=> $fitore);
    }
    ...
    public function singleStart(Request $request)
    {
        $maxPlayers = 1;
        $name = "singleGame";
        $level = (int) $request->level;
